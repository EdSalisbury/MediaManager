...
class MediaManager:
    ...
    def init_db(self):
        self.db = LocalDB(self.cfg.get('database_file'))
        self.db.__enter__()
        ...
        try:
            ...
            if db_path:
                ...
                if os.path.isfile(db_path):
                    ...
                    logger.debug(
                        f"Replacing record for {db_path} ({hash})")
    ...
    def import_file(self, path):
        try:
            ...
            if db_path and os.path.isfile(db_path):
                logger.info(
                    f"File has already been imported {path} ({hash}) {db_path}")
                return True
            else:
    ...
    def process_dir(self, folder, max_workers=4,
                    import_files=False, move_duplicates=False):
